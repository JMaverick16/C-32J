...
#	var att_fact = ((0.1 * (pressure_alt.getValue() - cabin)) / atten_m.getValue()) + atten_b;
	...
	# Final Descent Mode
	if (mode.getValue() == 2 and getprop("position/gear-agl-ft") < 2000) {
	    cabin_target.setValue(pressure_alt.getValue() - getprop("position/gear-agl-ft") - 100);
	}
}
...
#	    }
	...
	# Descent
	if (mode.getValue() == 2) {
	    if (getprop("position/gear-agl-ft") > 2000)
